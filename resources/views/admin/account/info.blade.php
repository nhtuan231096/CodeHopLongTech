	...
	<div class="row">
		<div class="col-md-2">
			<div class="avatar_info text-center">
				<img width="100%" src="{{url('uploads/admin')}}/{{Auth::Guard('admin')->user()->avatar}}" alt="Avatar admin">
				<p class="label label-primary">Cấp độ: {{Auth::Guard('admin')->user()->group->title}}</p>
					...
					<div class="row">
						...
						<thead>
							<tr>
								...
								<td>:</td>
								<td>{{Auth::Guard('admin')->user()->fullname}}</td>
							</tr>
						</thead>
						<tbody>
							<tr>
							...
							<tr>
								<th>Quyền admin</th>
								<td>:</td>
								<td>{{Auth::Guard('admin')->user()->group->title}}</td>
							</tr>
							<tr>
								<th></th>
								<td></td>
											...
											<div class="modal-body">
												<form action="{{route('changePassword')}}" method="POST" role="form">
													<div class="form-group">
														...
														<input type="password" class="form-control" name="oldPass" id="" placeholder="Mật khẩu của bạn" data-error="Mật khẩu không được để trống" required>
													</div>	
													@csrf
