...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="{{url('public/css')}}/newstyle.css" media="all" />
                                    ...
                                    <ul class="woocommerce-mini-cart cart_list product_list_widget ">
                                       @foreach($cart->items as $item)
                                        <li style="padding-top: 6px;padding-bottom: 6px" class="woocommerce-mini-cart-item mini_cart_item">
                                            ...
                                            <a href="{{route('view',['slug'=>$item['slug']])}}" style="font-weight: bold">
              ...
              <div class="container">
                <div class="shopping-cart">
                  <div class="shopping-cart-header">
                    <i class="fa fa-shopping-cart cart-icon"></i><span class="badge">{{$cart->total_quantity}}</span>
                    <div class="shopping-cart-total pull-right">
                      <span class="lighter-text">Total:</span>
                      <span class="main-color-text">{{number_format($cart->total_amount)}}</span>
                    </div>
                  </div> 

                  <ul class="shopping-cart-items" style="margin: 0">
                    @foreach($cart->items as $item)
                    <li class="clearfix">
                      <img style="max-width: 81px" src="{{url('uploads/product')}}/{{$item['image']}}" alt="{{$item['image']}}" />
                      <span class="item-name">{{$item['title']}}</span>
                      <span class="item-price">{{number_format($item['price'])}}</span> x
                      <span class="item-quantity">{{$item['quantity']}}</span>
                      <span class="pull-right">
                        <a style="padding: 0px 5px 2px 6px;border-radius: 50%;border: 1px solid;margin-right: 10%;color: red;font-weight: bold;" class="pull-right" href="{{route('delete_cart',['id'=>$item['id']])}}">x</a>
                      </span>
                    </li>
                    @endforeach
                  </ul>

                  <a href="{{route('view_cart')}}" class="button">Giỏ hàng</a>
                </div> 
              </div>  -->
              <!-- end cart -->
